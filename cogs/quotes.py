...
class QuotesCommands(commands.Cog):
    ...
    async def add_name(self, ctx, input_name: str) -> None:
        module_logger.info(f'Message command "add name" with input: [{input_name}] executed by {ctx.author.id}')
    ...
    async def slash_add_name(self, inter: disnake.CommandInteraction, name: str) -> None:
        module_logger.info(f'Message command "add name" with input: [{name}] executed by {inter.author.id}')
    ...
    async def slash_add_quote(self, inter: disnake.CommandInteraction, name: str, quote: str) -> None:
        module_logger.info(f'Slash command "add quote" with inputs: [{name}] [{quote}] executed by {inter.author.id}')
    ...
    async def slash_remove_name(self, inter, name: str) -> None:
        module_logger.info(f'Slash command "remove name" with inputs: [{name}] executed by {inter.author.id}')
        await inter.response.send_message(
